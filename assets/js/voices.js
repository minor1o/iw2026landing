...
/* ---------- farm voices ---------- */
  ...
  var whoBtns = Array.prototype.slice.call(document.querySelectorAll("#who button"));
  whoBtns.forEach(function(b){
    b.addEventListener("click", function(){
      whoBtns.forEach(function(x){ x.setAttribute("aria-selected", String(x === b)); });
      var q = quotes[+b.dataset.q];
      var t = document.getElementById("qText"), c = document.getElementById("qWho");
      t.style.opacity = 0; c.style.opacity = 0;
      setTimeout(function(){
        t.textContent = q[0]; c.innerHTML = q[1];
        t.style.transition = "opacity .3s"; c.style.transition = "opacity .3s";
        t.style.opacity = 1; c.style.opacity = 1;
      }, reduce ? 0 : 160);
    });
  });

  /* ---------- rivals comparison ---------- */
  var CAPS = ["Watches all night","Reads movement","Reads body temperature","Hears chewing and calling","Runs without a cloud account","Stays outside the animal"];
  var rivalData = [
    {n:"A person watching",
     ...
     miss:"Signs last six to eight hours and most of them fall at night. A third of cows barely show anything at all, and in hard frost the miss rate climbs to 83 percent.",
     caps:[0,0,0,0,1,1], price:"Cost: staff time, roughly one hour per checkup."},
    {n:"Hormone synchronisation",
     ...
     miss:"Around 44 percent of cows conceive in the published trials, it needs veterinary time, it works worse on poorly fed herds, and it is invasive by design.",
     caps:[0,0,0,0,1,0], price:"Cost: hormones, vet visits and repeated fixation of the animal."},
    {n:"Leg pedometer",
     how:"A tag on the leg counts steps and flags the cow when today's count jumps above her normal.",
     miss:"Silent heat has few extra steps, so the quiet third stays invisible. No feeding context, no sound, and the tags get lost in bedding and slurry.",
     caps:[1,1,0,0,1,1], price:"Cost: mid range hardware, plus replacements for the tags that disappear."},
    {n:"Ear tag analytics",
     ...
     miss:"The ear is a noisy place to measure from, temperature at the ear drifts with the weather, and the data lives in a subscription you do not control.",
     caps:[1,1,1,0,0,1], price:"Cost: per animal hardware plus an ongoing cloud subscription."},
    {n:"Rumen bolus",
     how:"A capsule sits in the rumen and measures core temperature precisely, with very little movement noise.",
     miss:"It has to be swallowed and is difficult to take back out. Battery life sets a hard expiry date, and it hears nothing at all.",
     caps:[1,0,1,0,1,0], price:"Cost: up to 27 euro per bolus for the cheap ones, far more for long life versions."},
    {n:"Vaginal probe",
     how:"A probe placed inside the cow reports temperature and the moment of heat directly.",
     miss:"Invasive, awkward to service and hard to scale across a herd. It measures no activity, so it tells you nothing about how she is behaving.",
     caps:[1,0,1,0,1,0], price:"Cost: hygiene and handling work on every animal, every cycle."},
    {n:"Imported neck collar",
     ...
     miss:"The model is tuned for herds that are not yours, there is no microphone, no battery indicator, and local adaptation and service run through an importer.",
     caps:[1,1,0,0,0,1], price:"Cost: 200 dollars and upward per collar, plus the base station and support contract."}
  ];
  var US = [1,1,1,1,1,1];
  function dot(on){ return '<span class="dot ' + (on ? "on" : "off") + '" role="img" aria-label="' + (on ? "yes" : "no") + '"></span>'; }
  function showRival(i){
    ...
    for (var k=0;k<CAPS.length;k++){
      rows += "<tr><td>" + CAPS[k] + "</td><td>" + dot(r.caps[k]) + "</td><td>" + dot(US[k]) + "</td></tr>";
    }
    document.getElementById("capBody").innerHTML = rows;
  }
  var rivalBtns = Array.prototype.slice.call(document.querySelectorAll("#rivalList button"));
  rivalBtns.forEach(function(b){
    b.addEventListener("click", function(){
      rivalBtns.forEach(function(x){ x.setAttribute("aria-selected", String(x === b)); });
